...
public class CNavigation {
    ...
    public CComputeBlockedEdge m_EdgeBlock_sightVer = new CComputeBlockedEdge(); // 가시성 고려 최단 거리 이동 모듈
    ...
    //@ -Return:
    public bool getPosWayTo_sightVer(int iSeqCell_current, Vector2[] arrv2PntObjBoundary, ref CCellWayto pntNavi_out)
    {
        if (iSeqCell_current > (int)m_arrNavicells.Length || iSeqCell_current < 0)
        {
        ...
        Vector3 v3PosDest = new Vector3();
        bool bGetPos_nextWay = searchNaviCell_sightVer(arrv2PntObjBoundary, ref naviCellCurr);
    ...
    //@ -Return:
    public bool getPosWayTo_sightVer(int iSeqCell_current, Vector2[] arrv2PntObjBoundary, ref Vector3 v3PosDest)
    {
        if (iSeqCell_current > (int)m_arrNavicells.Length || iSeqCell_current < 0)
        {
            Debug.Log("iSeqCell_current>(int)m_arrNavicells.size()||iSeqCell_current<0");
        ...
        //@ 가시성을 고려한 최단 거리의 위치값 설정.
        searchNaviCell_sightVer(arrv2PntObjBoundary, ref naviCellCurr);

        return naviCellCurr.getPos_nextWay(ref v3PosDest);
    } // public bool getPosWayTo_sightVer(	int iSeqCell_current, ref Vector3 v3PosDest )
    ...
    static public void collectWayToGoalLinear(CNAVICELL[] arrNavicells_,
                                                ...
                                                ref CComputeBlockedEdge processorEdge_)
    {
        bool bOrganizedComplete = false;
        Vector3 pntBlockedEdge0 = Vector3.zero;
        Vector3 pntBlockedEdge1 = Vector3.zero;
        ...
        while (false == bOrganizedComplete)
        {
            ...
            for (int iSeqAdj = 0; iSeqAdj < cellWay.m_arriAdjCells.Length; ++iSeqAdj)
            {
                ...
                //@ Checkup adjacent Edge Tri only case of type is Null Cell.
                if (CNAVICELL.NULL_CELL == iIdxAdj)
                {
                    triWaytoGoal.GetEdgePoint(eEdge, ref pntBlockedEdge0, ref pntBlockedEdge1);
                    processorEdge_.AddToBlockEdge(pntBlockedEdge0, pntBlockedEdge1);       //-> Add Edge to Block List
                }
                else
                {
                    ...
                    if (cellAdj.IsBlockCell() || cellAdj.IsBlockRoadCell())
                    {
    ...
    //@ search intersected point // More faster 3.x~4.x times average than RayIntersect.
    ...
    public bool searchNaviCell_sightVer(Vector2[] arrv2PntObjBoundary, ref CNAVICELL cellWayStart)
    {
        ...
        collectWayToGoalLinear(m_arrNavicells, cellWayStart, ref listSequenceCell, ref m_EdgeBlock_sightVer);
        ...
        foreach (CNAVICELL cellCurr in listSequenceCell)
        {
            ...
            {
                if (m_EdgeBlock_sightVer.crossBlockIteration(v3PntCell, v3PntNext))
                {
            ...
            {
                //@ Cross At least 1 mo
                if (true == m_EdgeBlock_sightVer.crossBlockIteration(v2PntObj, v2PntNext))
                {
                    bCrosswithBlock = true;
                    break;
                }
            ...
            //testdebug
            Debug.Log("skipped navicell sight cal/" + System.DateTime.Now.Millisecond);
        } // foreach (CNAVICELL cellCurr in listSequenceCell)
    ...
    } // public bool searchNaviCell_sightVer(int iSeqCell)
        ...
        foreach (int iSeqTriStart in tricollector.m_listTris_naviStart)
        {
            ...
            if (listSequenceCell.Count > 2)
            {
                ...
                foreach (CNAVICELL cellCurr in listSequenceCell)
                {
                    ...
                    bCrosswithBlock = m_EdgeBlock_sightVer.crossBlockIteration(v3PntStart_, v3PntEnd_);
                    if (true == bCrosswithBlock)
                    {
    ...
    {
        m_EdgeBlock_sightVer.InitEdgeBlock();

        BuildupNavicellsFromTris(listTris_);
    ...
    protected void buildAdjacentNavicells(ref CNAVICELL[] arrNavicells)
    {
        for (int iSeqCell = 0; iSeqCell < arrNavicells.Length; ++iSeqCell)
        {
            CNAVICELL naviCell = (arrNavicells[iSeqCell]);
            buildAdjacentANavicell(ref naviCell, ref arrNavicells);
        }
    ...
    protected bool buildAdjacentANavicell(ref CNAVICELL cellLeft,
                                        ref CNAVICELL[] arrNavicells)
    {
        ...
        return true;
    } // protected bool buildAdjacentANavicell



    // 함수:BuildupNavicellsFromTris 
    // 네비게이션을 위한 모든 셀들과 인접 셀들을 생성한다
    //@ Construct all adjacent Cells for navigation.
    protected void BuildupNavicellsFromTris(List<CTRI> listTris_)
    {
        ...
        for (int iTri = 0; iTri < iCntTris; iTri++)
        {
            ...
            naviCell.ToNavicellDataFromTri(iTri, triCurr);   // Tri 기본 정보를 네비셀에 전달 그리고 각각의 미들 엣지,센터를 계산
            m_arrNavicells[iTri] = naviCell;
        }

        buildAdjacentNavicells(ref m_arrNavicells); // 인접 셀 빌드업
    }
    ...
    protected void setClearAllcell_unprocessed(ref CNAVICELL[] arrNavicells)
    {
        ...
        //@ Block Edge Clear
        m_EdgeBlock_sightVer.Release_Edge();
    }
    ...
    public bool mappingNavigation(List<int> listCellsGoal, List<int> listCellsBlock, List<int> listCellsBlockRoad, List<int> listCellsStart, bool bRequestSetStandardLevel)
    {
        ...
        if (true == bRequestSetStandardLevel)
        {
            ...
            //For PROFILING
            //UnityEngine.Profiler.logFile = "aa.txt";q
